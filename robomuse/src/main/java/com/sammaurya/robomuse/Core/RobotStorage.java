...
public class RobotStorage {
    ...
    private static List<RobotInfo> mRobotInfos = Lists.newArrayList();
    private static Gson mGson = new Gson();
    private static TreeMap<String, String> mPrefKeyMap = new TreeMap<>();
    ...
    public static synchronized void load(Activity activity) {
        ...
        String defaultJson = mGson.toJson(new ArrayList<RobotInfo>());

        String robotInfoJson = pref.getString(ROBOT_INFOS_KEY, defaultJson);
        Type listOfRobotInfoType = new TypeToken<List<RobotInfo>>() {
        }.getType();

        mRobotInfos = mGson.fromJson(robotInfoJson, listOfRobotInfoType);

        RobotInfo.resolveRobotCount(mRobotInfos);

        mPrefKeyMap.put(RobotInfo.JOYSTICK_TOPIC_KEY, activity.getString(R.string.prefs_joystick_topic_edittext_key));
        mPrefKeyMap.put(RobotInfo.CAMERA_TOPIC_KEY, activity.getString(R.string.prefs_camera_topic_edittext_key));
        mPrefKeyMap.put(RobotInfo.LASER_SCAN_TOPIC_KEY, activity.getString(R.string.prefs_laserscan_topic_edittext_key));
        mPrefKeyMap.put(RobotInfo.NAVSAT_TOPIC_KEY, activity.getString(R.string.prefs_navsat_topic_edittext_key));
        mPrefKeyMap.put(RobotInfo.ODOMETRY_TOPIC_KEY, activity.getString(R.string.prefs_odometry_topic_edittext_key));
        mPrefKeyMap.put(RobotInfo.POSE_TOPIC_KEY, activity.getString(R.string.prefs_pose_topic_edittext_key));
        mPrefKeyMap.put(RobotInfo.REVERSE_LASER_SCAN_KEY, activity.getString(R.string.prefs_reverse_angle_reading_key));
        mPrefKeyMap.put(RobotInfo.INVERT_X_KEY, activity.getString(R.string.prefs_invert_x_axis_key));
        mPrefKeyMap.put(RobotInfo.INVERT_Y_KEY, activity.getString(R.string.prefs_invert_y_axis_key));
        mPrefKeyMap.put(RobotInfo.INVERT_ANGULAR_VELOCITY_KEY, activity.getString(R.string.prefs_invert_angular_velocity_key));
    }

    public static String getPreferenceKey(String bundleKey) {
        return mPrefKeyMap.get(bundleKey);
    }
    ...
    public static synchronized boolean remove(Activity activity, RobotInfo robot) {
        boolean removed = mRobotInfos.remove(robot);
        save(activity);
        return removed;
    }
    ...
    public static synchronized RobotInfo remove(Activity activity, int index) {
        RobotInfo removed = mRobotInfos.remove(index);
        save(activity);
        return removed;
    }
    ...
    public static synchronized boolean update(Activity activity, RobotInfo robot) {
        ...
        for (int i = 0; i < mRobotInfos.size(); i++) {

            if (mRobotInfos.get(i).compareTo(robot) == 0) {

                Log.d(TAG, "Updating robot info at position " + i + ": " + robot);
                Log.d(TAG, robot.getOdometryTopic());

                mRobotInfos.set(i, robot);
                save(activity);
                updated = true;
                break;
            }
    ...
    public static synchronized boolean add(Activity activity, RobotInfo robot) {
        boolean added = mRobotInfos.add(robot);
        save(activity);
        return added;
    }
    ...
    public static synchronized void save(Activity activity) {
        String robotInfosJson = mGson.toJson(mRobotInfos);
